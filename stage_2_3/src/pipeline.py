...
class StableSyncMVDPipeline(StableDiffusionControlNetPipeline):
	...
	def __call__(
		...
		images_sr_rgb = np.hstack([np.array(img.resize((1024, 1024))) for img in image]).astype(np.float32) / 255.
		textured_views_rgb = np.vstack([images_sr_rgb, textured_views_rgb[0]])[None,...]
