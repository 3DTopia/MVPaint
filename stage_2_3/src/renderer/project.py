...
class UVProjection():
	...
	def load_glb_mesh(self, mesh_path, scale_factor=1.0, auto_center=True, autouv=False):
	...
	@torch.no_grad()
	def get_point_validation_by_o3d(self, points, eye_position, hidden_point_removal_radius=200):
		...
		camera_pose = eye_position.get_camera_center().squeeze().cpu().numpy().astype(np.float64)
	...
	def hidden_judge(self, camera, texture_dim):
		...
		uv_position, face_idx = kal.render.mesh.rasterize(texture_dim, texture_dim, face_vertices_z,uv_face_attr * 2 - 1,face_features=face_vertices_world, )
		...
		points_visibility = self.get_point_validation_by_o3d(valid_points, camera).float()
		visibility_map = torch.zeros((texture_dim * texture_dim, )).cuda()
	...
	def bake_texture(self, views=None, main_views=[], cos_weighted=True, channels=None, exp=None, noisy=False,
					 generator=None, smooth_colorize=False):
		if not exp:
			exp = 1
		if not channels:
			channels = self.channels
		...
		for i in range(len(bake_maps)):
			...
			if smooth_colorize:
				visibility_map = self.hidden_judge(self.cameras[i], self.target_size[0]).unsqueeze(-1)
		...
		if not smooth_colorize:
			...
			tmp_mesh.textures = TexturesUV([baked], tmp_mesh.textures.faces_uvs_padded(),
										   tmp_mesh.textures.verts_uvs_padded(), sampling_mode=self.sampling_mode)
