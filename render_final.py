...
def render(in_mesh_path, out_folder, obj_name):
    options = [
    ...
    mesh = MeshLoader.load(in_mesh_path, glctx, front_dir='-y', resize=True, clean=False, 
                            renormal=False, bound=0.8, retex=False, retex_options=options[0])
    ...
    for res in resolutions:
        ...
        ele = -30 
        start = 0
        for mode, ext in zip(['albedo'], ['png']):
            renderer.mode = mode
            images = []
            ...
            imageio.imwrite(f'{out_folder}/{obj_name}_{mode}_{res:03d}.{ext}', img)
...
if __name__ == "__main__":
    ...
    in_obj_filenames = sorted([x for x in os.listdir(in_root) if os.path.isdir(os.path.join(in_root, x))])
    print("==== obj total", len(in_obj_filenames))
    for obj_id, obj_name in tqdm(enumerate(in_obj_filenames)):
        in_mesh_path = os.path.join(in_root, obj_name, "final/textured.obj")

        print("=== rendering", obj_name, flush=True)
        render(in_mesh_path, out_root, obj_name)
        # break 
