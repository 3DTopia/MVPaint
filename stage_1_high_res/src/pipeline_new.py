...
@torch.no_grad()
def stable_upscale(low_res_texture, prompt, negative_prompt, guidance_scale=None, num_inference_steps=None,
...
class StableSyncMVDPipeline(StableDiffusionControlNetPipeline):
	...
	def __call__(
		...
		print("encode_prompt", prompt_embeds.shape)
		...
		low_res_image = crop(image_path, 256, 768)
			...
			for i, t in enumerate(timesteps):
				...
				for prompt_tag, prompt_embeds in prompt_embeds_groups.items():
					if prompt_tag == "positive" or not guess_mode:
						...
						for model_input_batch ,prompt_embeds_batch, conditioning_images_batch, conditioning_images_batch2 \
							in zip (model_input_batches, prompt_embeds_batches, conditioning_images_batches, conditioning_images_batches2):
							print("controlnet", model_input_batch.shape, prompt_embeds_batch.shape, conditioning_images_batch.shape, conditioning_images_batch2.shape)
		...
		upscaled_texture = stable_upscale(result_tex_rgb,
		...
		print(result_tex_rgb.shape)
		self.uvp_rgb.set_texture_map(result_tex_rgb.permute(2,0,1))
		print(self.uvp_rgb.mesh.textures.maps_padded().shape)
		position_map = self.UV_pos_render()
		...
		images_load_rgb = np.hstack([np.array(img.resize((1024, 1024))) for img in low_res_image]).astype(np.float32) / 255.
