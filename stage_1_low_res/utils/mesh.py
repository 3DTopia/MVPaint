...
class Mesh:
    ...
    def load_obj(cls, path, albedo_path=None, device=None):
        ...
        if not path.startswith('s3://'):
            ...
            with smart_open(os.path.dirname(path)+'.tar', 'rb') as f:
                tar_content = f.read()
                tar_stream = io.BytesIO(tar_content)
                with tarfile.open(fileobj=tar_stream, mode='r') as tar:
                    file_obj = tar.extractfile(os.path.basename(path))
                    lines = file_obj.read().decode('utf-8').split('\n')
        ...
        if not use_vertex_color:
            ...
            if mtl_path is not None and albedo_path is None:
                # load obj
                if not mtl_path.startswith('s3://'):
                    with open(mtl_path, "r") as f:
                        lines = f.readlines()
                else:
                    with smart_open(os.path.dirname(mtl_path)+'.tar', 'rb') as f:
                        tar_content = f.read()
                        tar_stream = io.BytesIO(tar_content)
                        with tarfile.open(fileobj=tar_stream, mode='r') as tar:
                            file_obj = tar.extract(os.path.basename(mtl_path))
...
if __name__ == "__main__":
    meshroot = "s3://chengwei/data/objs_xatlas"
