...
def t2i(model, image_size, prompt, uc, sampler, step=20, scale=7.5, batch_size=8, ddim_eta=0., dtype=torch.float32, device="cuda", camera=None, num_frames=1):
    ...
    with torch.no_grad(), torch.autocast(device_type=device, dtype=dtype):
        ...
        samples_ddim, _, _, _, _ = sampler.sample(S=step, conditioning=c_,
...
if __name__ == "__main__":
    ...
    uc = model.get_learned_conditioning( ["strong light, Bright light, intense light, dazzling light, brilliant light, radiant light, Shade, darkness, silhouette, dimness, obscurity, shadow, glasses"] ).to(device)
